...
def setup_logger(method: str, today: str):
    global logger
    if logger.hasHandlers():
        logger.handlers.clear()
    log_dir = Path(__file__).resolve().parent / "out/logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger(__name__)
    logger.setLevel(logging.DEBUG)
    formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
    file_handler1 = logging.FileHandler(log_dir / f"{today}_train_{method}_info.log")
    file_handler1.setLevel(logging.INFO)
    file_handler1.setFormatter(formatter)
    file_handler2 = logging.FileHandler(log_dir / f"{today}_train_{method}_debug.log")
...
def hyper_parameter(length: int, method: str, n_samples: int | None = None) -> dict:
    if method == "gpr":
        kernels = [
        ...
        return {"gpr__kernel": kernels}
    if method in {"ols", "blr"}:
        return {}
    if method in {"ridge", "lasso"}:
        return {f"{method}__alpha": np.logspace(-4, 4, 50)}
    ...
    if method == "pls":
        max_comp = (
            ...
            else min(7, length, max(1, n_samples - 2))
        )
        return {"pls__n_components": list(range(1, max_comp + 1))}
    if method == "svr":
        return {
...
def get_clf(
    length: int, cv: Any, method: str, n_samples: int | None = None
) -> GridSearchCV:
    pipeline = Pipeline([("scaler", StandardScaler()), (method, get_method(method))])
    scoring = {
    ...
    return GridSearchCV(
        pipeline,
        hyper_parameter(length, method, n_samples),
        cv=cv,
        scoring=scoring,
        n_jobs=-1,
        refit="mae",
...
def best_combination(
    ...
    inner_cv = RepeatedKFold(
        n_splits=inner_n_splits, n_repeats=inner_n_repeats, random_state=random_state
    )
    ...
    for combi in combinations:
        ...
        logger.debug(
            f"{combi}: mae -> {row['inner_cv_mae']}, rmse -> {inner_cv_rmse}, r2 -> {inner_cv_r2}"
        )
...
def nested_cv_evaluate(
    ...
    outer_cv = get_outer_cv(
        cv_type, len(X), outer_n_splits, outer_n_repeats, random_state
    )
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        y_pred = estimator.predict(X_test_all[combi])
    ...
    selection_summary = {
        "feature_frequency": {
            k: int(v) for k, v in selected_descriptor_counter.items()
        },
        "selected_set_frequency": {
            ",".join(k): int(v) for k, v in selected_set_counter.items()
        },
    }
    return pred_df, metrics, selection_summary
...
def permutation_test(
    ...
    for i in range(n_permutations):
        y_permuted = pd.Series(rng.permutation(y.values), index=y.index, name=y.name)
        logger.info(f"permutation test {i + 1}/{n_permutations}")
        _, perm_metrics, _ = nested_cv_evaluate(
            X,
            y_permuted,
            descriptors,
            method,
...
def evaluate_external_test(
    estimator, external_df: pd.DataFrame, target: str, selected_descriptors: list[str]
) -> tuple[pd.DataFrame, dict]:
    X_external = external_df[selected_descriptors]
    y_external = external_df[target]
    y_pred = estimator.predict(X_external)
...
def train(
    ...
    setup_logger(method, today)
    start_time = time.time()
    if target not in TARGET or method not in METHOD:
        raise ValueError(f"{target} or {method} is not exist.")
    if not any([run_loo, run_kfold, run_repeated_kfold]):
        raise ValueError(
            "At least one of run_loo, run_kfold, run_repeated_kfold must be True."
        )
    ...
    all_prediction_dfs, all_metrics, all_selection_summaries = [], {}, {}
    cv_settings = [
        ("loo", run_loo),
        ("kfold", run_kfold),
        ("repeated_kfold", run_repeated_kfold),
    ]
    for cv_type, should_run in cv_settings:
        if not should_run:
            continue
        pred, metrics, sel = nested_cv_evaluate(
            X,
            y,
            descriptors,
            method,
        ...
        all_prediction_dfs.append(pred)
        all_metrics[cv_type] = metrics
        all_selection_summaries[cv_type] = sel
    pred_df = pd.concat(all_prediction_dfs, ignore_index=True)

    permutation_result = None
    if run_permutation_test:
        if "repeated_kfold" in all_metrics:
            observed_mae, permutation_cv_type = (
                all_metrics["repeated_kfold"]["mae"],
                "repeated_kfold",
            )
        elif "loo" in all_metrics:
            observed_mae, permutation_cv_type = all_metrics["loo"]["mae"], "loo"
        else:
            first_key = next(iter(all_metrics))
            observed_mae, permutation_cv_type = all_metrics[first_key]["mae"], first_key
    ...
    final_best, final_best_combi = best_combination(
        X, y, final_combinations, method, inner_n_splits, inner_n_repeats, random_state
    )
    ...
    if external_df is not None:
        ...
        external_pred_df, external_metrics = evaluate_external_test(
            final_best["best_estimator"],
            external_df,
            target,
            final_selected_descriptors,
        )
        logger.info(f"external_test_metrics: {external_metrics}")

    out_dir = Path(__file__).resolve().parent / "out"
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_pred_path = out_dir / f"{today}_{method}_predictions.csv"
    json_summary_path = out_dir / f"{today}_{method}_summary.json"
    pkl_estimator_path = out_dir / f"{today}_{method}_final_estimator.pkl"
    csv_external_pred_path = out_dir / f"{today}_{method}_external_predictions.csv"
