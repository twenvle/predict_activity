...
def setup_logger(method: str, today: str, name: str, target: str, cv_type: str):
    global logger
    ...
    file_handler1 = logging.FileHandler(
        f"{log_dir}/{today}_{method}_{name}_{target}_{cv_type}info.log"
    )
    file_handler1.setLevel(logging.INFO)

    file_handler2 = logging.FileHandler(
        f"{log_dir}/{today}_{method}_{name}_{target}_{cv_type}debug.log"
    )
...
def get_method(method: str):
    methods = {
        # normalize_y=True にすると、GPR内部では y を標準化して学習し、predict は元のスケールに戻します。
        # 0-100のパーセント値を直接扱いたい場合は、必要に応じて False に戻してください。
...
def hyper_parameter(length: int, method: str, n_samples: int = None) -> dict:
    if method == "gpr":
        ...
        alpha = np.logspace(-4, 4, 50)
        # l1_ratio=0 は Ridge と重複し、ElasticNet では警告が出やすいため除外
...
def get_clf(length: int, cv: Any, method: str, n_samples: int = None) -> GridSearchCV:
    pipeline = Pipeline(
        [
...
def train(
    ...
    cv_type = "loo_" if run_loo else ""
    cv_type += "repeated_kfold_" if run_repeated_kfold else ""
    cv_type += "kfold_" if run_kfold else ""
    setup_logger(method, today, name, target, cv_type)
    ...
    if run_loo:
        loo_pred_df, loo_metrics, loo_selection_summary = nested_cv_evaluate(
            X=X,
            y=y,
            descriptors=descriptors,
            method=method,
        ...
        all_prediction_dfs.append(loo_pred_df)
        all_metrics["loo"] = loo_metrics
        all_selection_summaries["loo"] = loo_selection_summary

    if run_kfold:
        kfold_pred_df, kfold_metrics, kfold_selection_summary = nested_cv_evaluate(
            X=X,
            y=y,
            descriptors=descriptors,
            method=method,
        ...
        all_prediction_dfs.append(kfold_pred_df)
        all_metrics["kfold"] = kfold_metrics
        all_selection_summaries["kfold"] = kfold_selection_summary

    if run_repeated_kfold:
        repeated_pred_df, repeated_metrics, repeated_selection_summary = (
            nested_cv_evaluate(
                X=X,
                y=y,
                descriptors=descriptors,
        ...
        all_prediction_dfs.append(repeated_pred_df)
        all_metrics["repeated_kfold"] = repeated_metrics
        all_selection_summaries["repeated_kfold"] = repeated_selection_summary
    ...
    csv_pred_path = (
        out_dir / f"{today}_{method}_{name}_{target}_{cv_type}predictions.csv"
    )
    json_summary_path = (
        out_dir / f"{today}_{method}_{name}_{target}_{cv_type}summary.json"
    )
    pkl_estimator_path = (
        out_dir / f"{today}_{method}_{name}_{target}_{cv_type}final_estimator.pkl"
    )
    csv_external_pred_path = (
        out_dir / f"{today}_{method}_{name}_{target}_{cv_type}external_predictions.csv"
    )
