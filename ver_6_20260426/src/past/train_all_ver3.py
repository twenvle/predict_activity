...
def setup_logger(file_name: str):
    ...
    file_handler1 = logging.FileHandler(f"{log_dir_debug}/{file_name}_info.log")
    file_handler1.setLevel(logging.INFO)

    file_handler2 = logging.FileHandler(f"{log_dir_info}/{file_name}_debug.log")
...
def save_checkpoint(predictions, checkpoint_path: Path):
...
def train(
    ...
    file_name = f"{os.path.basename(__file__)}_{method}_{name}_{target}_{run}"
    ...
    run_pred_df, run_metrics, run_selection_summary = nested_cv_evaluate(
        X=X,
        y=y,
        descriptors=descriptors,
        method=method,
