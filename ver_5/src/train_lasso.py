...
file_handler1 = logging.FileHandler(f"{log_dir}/{today}_train_lasso_info.log")
file_handler1.setLevel(logging.INFO)

file_handler2 = logging.FileHandler(f"{log_dir}/{today}_train_lasso_debug.log")
...
def hyper_parameter() -> dict:
    ...
    param_grid = {"lasso__alpha": alpha.tolist()}
    return param_grid
...
def inner_cv(
    ...
    best_model = clf.best_estimator_.named_steps["lasso"]
    row = {
...
def train(df: pd.DataFrame, descriptors: list = DESCRIPTORS, target: str = "yield"):
    ...
    pred_path = Path(__file__).resolve().parent / f"out/{today}_lasso_predictions.csv"
    summary_path = Path(__file__).resolve().parent / f"out/{today}_lasso_summary.json"
