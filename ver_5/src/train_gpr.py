...
file_handler1 = logging.FileHandler(f"{log_dir}/{today}_train_gpr_info.log")
file_handler1.setLevel(logging.INFO)

file_handler2 = logging.FileHandler(f"{log_dir}/{today}_train_gpr_debug.log")
...
def hyper_parameter(length: int) -> dict:
    kernels = [
...
def get_clf(length: int, cv: Any) -> GridSearchCV:
    pipeline = Pipeline(
        [
    ...
    clf = GridSearchCV(
        estimator=pipeline,
        param_grid=hyper_parameter(length=length),
        cv=cv,
        scoring=scoring,
        n_jobs=-1,
        refit="mae",
...
def best_combination(
    ...
    for combi in combinations:
        ...
        clf = get_clf(len(combi), inner_cv)
        ...
        row = {
            ...
            "best_kernel": str(clf.best_estimator_.named_steps["gpr"].kernel_),
        }
        logger.debug(f"{combi}: kernel -> {row['best_kernel']}")
        logger.debug(
    ...
    logger.info(f"selected_kernel: {best['best_kernel']}")
...
def train(df: pd.DataFrame, descriptors: list = DESCRIPTORS, target: str = "yield"):
    ...
    logger.info(f"selected_kernel: {final_summary['selected_kernel']}")
