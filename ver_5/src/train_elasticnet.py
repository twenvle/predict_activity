...
file_handler1 = logging.FileHandler(f"{log_dir}/{today}_train_elascitnet_info.log")
file_handler1.setLevel(logging.INFO)

file_handler2 = logging.FileHandler(f"{log_dir}/{today}_train_elascitnet_debug.log")
...
def hyper_parameter() -> dict:
    ...
    param_grid = {
        "elascitnet__alpha": alpha.tolist(),
        "l1_ratio": l1_ratio.tolist(),
    }
    return param_grid
...
def get_clf(cv: Any) -> GridSearchCV:
    pipeline = Pipeline(
        [
            ...
            ("elascitnet", ElasticNet(random_state=42)),
        ]
    )
    scoring = {
...
def inner_cv(
    ...
    best_model = clf.best_estimator_.named_steps["elascitnet"]
    row = {
...
def train(df: pd.DataFrame, descriptors: list = DESCRIPTORS, target: str = "yield"):
    ...
    pred_path = (
        Path(__file__).resolve().parent / f"out/{today}_elascitnet_predictions.csv"
    )
    summary_path = (
        Path(__file__).resolve().parent / f"out/{today}_elascitnet_summary.json"
    )
