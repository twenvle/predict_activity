...
file_handler1 = logging.FileHandler(f"{log_dir}/{today}_train_ols_info.log")
file_handler1.setLevel(logging.INFO)

file_handler2 = logging.FileHandler(f"{log_dir}/{today}_train_ols_debug.log")
...
def train(df: pd.DataFrame, descriptors: list = DESCRIPTORS, target: str = "yield"):
    ...
    pred_path = Path(__file__).resolve().parent / f"out/{today}_ols_predictions.csv"
    summary_path = Path(__file__).resolve().parent / f"out/{today}_ols_summary.json"
