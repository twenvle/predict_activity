...
file_handler1 = logging.FileHandler(f"{log_dir}/{today}_train_ridge_info.log")
file_handler1.setLevel(logging.INFO)

file_handler2 = logging.FileHandler(f"{log_dir}/{today}_train_ridge_debug.log")
...
def hyper_parameter() -> dict:
    ...
    param_grid = {"ridge__alpha": alpha.tolist()}
    return param_grid
...
def train(df: pd.DataFrame, descriptors: list = DESCRIPTORS, target: str = "yield"):
    ...
    pred_path = Path(__file__).resolve().parent / f"out/{today}_ridge_predictions.csv"
    summary_path = Path(__file__).resolve().parent / f"out/{today}_ridge_summary.json"
