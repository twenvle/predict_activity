...
_SIGMA_FLOOR = 1e-12
_VALID_METHODS = {"gpr", "ols", "ridge", "lasso", "elasticnet", "svr", "bayesian_ridge"}
...
def compute_ad_penalty(ad_ratio, mode: AdMode):
    ...
    raise ValueError(f"ad_mode='{mode}' は無効です。'none' / 'hard' / 'soft' のいずれかを指定してください。")
...
def _predict_mu_sigma(
    ...
    """
    if method in {"gpr", "bayesian_ridge"}:
        ...
        rmse = float(np.std(y_train - y_train_pred, ddof=1))
...
def predict(
    ...
    expected_result_cols = {
        "mu", "sigma_raw", "sigma_cal", "ci_lower", "ci_upper",
        "pi", "ei", "ucb", "ad_knn_distance", "ad_ratio", "in_ad", "ad_penalty", "ei_ad",
    }
    candidate_cols = set(candidate_df.columns) | expected_result_cols | {"prediction_status", "known_any"}
    ...
    if len(valid_idx) > 0:
        ...
        # モデル別 mu / sigma 取得
        mu, sigma_raw = _predict_mu_sigma(method, model, X_query_scaled, X_train_scaled, y_train)
    ...
    return output_df, output_path  # ← 修正: 保存先を返す